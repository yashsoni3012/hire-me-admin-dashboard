...
//             const val = item.is_status !== undefined ? item.is_status : item.status;
//             return val === true || val === "true" || val === 1 || val === "1" || val === "active";
//           };

//           const formData = {
//             name: data.name || "",
//             status: isStatusActive(data) ? "active" : "inactive",
...
//       type: "text",
//       required: true,
//       placeholder: "e.g. Private Limited, Public, Partnership",
//       help: "Enter the company type name",
//     },
//     {
//       name: "status",
//       label: "Status",
...
//     name: {
//       required: true,
//       requiredMessage: "Company type name is required",
//       minLength: 2,
//       minLengthMessage: "Company type name must be at least 2 characters",
//       maxLength: 100,
//       maxLengthMessage: "Company type name must be at most 100 characters",
//     },
//   };
...
//       showSuccess("Company type updated successfully");
//       navigate('/company-types');
//     } catch (error) {
//       console.error('Submit error:', error);
//       showError(error.message || "Failed to update company type");
//     } finally {
//       setLoading(false);
//     }
//   };
...
//       showError(error.message || "Failed to delete company type");
//     } finally {
...
//           <p className="text-sm text-gray-400">Loading company type data...</p>
//         </div>
//       </div>
//     );
//   }
...
//       breadcrumb={`Editing: ${editItem?.name || 'Company Type'}`}
//       showEdit={false}
//     />
//   );
// };
...
// pages/company-types/EditCompanyType.jsx
import React, { useState, useEffect } from 'react';
import { useNavigate, useParams } from 'react-router-dom';
...
const EditCompanyType = () => {
  ...
  const validateField = (name, value) => {
    if (name === 'name') {
      if (!value || !value.trim()) return 'Company type name is required';
      if (value.trim().length < 2) return 'Company type name must be at least 2 characters';
      if (value.trim().length > 100) return 'Company type name must be at most 100 characters';
    }
    return null;
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (/foreign\s*key|constraint|used|referenced/i.test(message)) {
        showError('Cannot delete this company type because it is being used in other records.');
      } else {
        showError(message || 'Failed to delete company type');
      }
    } finally {
      setDeleteLoading(false);
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-[#F4F5FA]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-slate-400">Loading company type data...</p>
        </div>
      </div>
    );
  }
  ...
  // ─── Compute hero data ────────────────────────────────────
  const companyTypeName = formValues.name?.trim() || 'Company Type';
  ...
  return (
    <div className="min-h-screen pb-16 bg-[#F4F5FA]">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <button
              ...
              aria-label="Delete company type"
            ...
            >
              ...
              {loading ? 'Updating...' : 'Update Company Type'}
            </button>
          </div>
        </div>
      </div>
            ...
            {TABS.map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  {active && (
                    <motion.span
                      layoutId="edit-company-type-tab-underline"
      ...
      <ConfirmDialog
        ...
        title="Delete Company Type"
        message={`Delete company type "${companyTypeName}"? This action cannot be undone.`}
      />
    </div>
  );
};
