...
const CompanyFaq = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      const r = await companyFaqService.getAll({ limit: 1000 });
      let rawData = [];
      if (r?.data?.data) {
        rawData = r.data.data;
      } else if (r?.data) {
      ...
      // Sort by order, then alphabetically by question for same order
      ...
      setData(sortedFaqs);
      console.log(`📊 Loaded ${sortedFaqs.length} Company FAQs`);
    } catch (error) {
      console.error("Load error:", error);
      showError(error.message || "Failed to load Company FAQs");
    } finally {
      setLoading(false);
    }
  };
  ...
  // Navigation handlers
  const openAdd = () => {
    navigate("/company-faq/add");
  };

  const openEdit = (item) => {
    navigate(`/company-faq/edit/${item.id}`);
  };

  const openView = (item) => {
    navigate(`/company-faq/view/${item.id}`);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await companyFaqService.delete(deleteId);
      ...
      if (/foreign\s*key|constraint|used|referenced/i.test(message)) {
        showError(
          "Cannot delete this FAQ because it is being used in other records.",
        );
      } else {
        showError(message || "Failed to delete Company FAQ");
      }
    } finally {
      setDeleteId(null);
  ...
  const handleStatusToggle = async (row) => {
    ...
    try {
      ...
      await companyFaqService.update(row.id, updateData);
      showSuccess(
  ...
  const handleTrendingToggle = async (row) => {
    ...
    try {
      ...
      setData((prev) =>
        prev.map((item) =>
          item.id === row.id ? { ...item, is_trending: currentTrending } : item,
        ),
      );
      showError(
        error.response?.data?.message ||
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Company FAQs</h1>
          <p className="text-sm text-gray-500 mt-1">
            Manage frequently asked questions for companies
          </p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <Table
          ...
          data={paginatedData}
          loading={loading}
          emptyMessage="No Company FAQs found"
        />
