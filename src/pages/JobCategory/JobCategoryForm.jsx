// // pages/jobs/JobCategoryForm.jsx
...
//                     if (!item) {
//                         const response = await jobCategoryService.getById(id);
//                         item = response.data;
//                     }
...
//                     showError("Failed to load job category data");
...
//             const allData = await jobCategoryService.getAll({ limit: 1000 });
//             const existingItems = allData.data?.data || allData.data?.results || allData.data || [];
//             const duplicate = existingItems.some(item =>
//                 (item.category_name || item.name || "").toLowerCase() === formData.name.toLowerCase() &&
//                 (mode === 'add' || item.id !== id)
//             );
...
//         try {
//             await jobCategoryService.delete(id);
...
//     const getTitle = () => {
//         if (mode === 'view') return 'Job Category Details';
//         if (mode === 'edit') return 'Edit Job Category';
//         return 'Add New Job Category';
//     };
...
//             <div className="min-h-screen bg-gray-50 flex items-center justify-center">
//                 <div className="text-center">
//                     <p className="text-gray-500">Job category not found</p>
//                     <button
...
//             onEdit={() => navigate(`/job-categories/edit/${id}`, { state: { item: data } })}
//         />
//     );
// };
...
const JobCategoryForm = () => {
      ...
      if ((mode === 'edit' || mode === 'view') && id) {
        ...
        try {
          ...
          if (normalizedData.image) {
            setImagePreview(getFullImageUrl(normalizedData.image));
          }
        } catch (error) {
          console.error('Fetch error:', error);
          showError('Failed to load job category data');
  ...
  const handleBack = () => navigate('/job-categories');
  ...
  const renderField = (field) => {
    ...
    if (isViewMode) {
      ...
      } else if (name === 'image' && imagePreview) {
        display = (
          <div className="relative group inline-block">
            <img
              src={imagePreview}
              alt="Category image"
              className="w-32 h-32 rounded-lg object-cover border border-slate-200 shadow-sm"
              onError={(e) => { e.target.style.display = 'none'; }}
            />
            <button
    ...
    switch (type) {
      case 'file': {
        ...
        const removeFn = isIcon ? handleRemoveIcon : handleRemoveImage;
        const changeFn = isIcon ? handleIconChange : handleImageChange;
        const inputId = isIcon ? 'icon-upload' : 'image-upload';
        const rounded = isIcon ? 'rounded-full' : 'rounded-lg';
        const size = isIcon ? 'w-20 h-20' : 'w-32 h-20';
              ...
              {preview ? (
                <div className="relative group">
                  <img
                    src={preview}
                    alt={label}
                    className={`${size} object-cover ${rounded} border-2 border-slate-200 shadow-sm`}
                  />
                  <button
                    type="button"
                    onClick={removeFn}
                ...
                <div className={`${size} bg-slate-100 ${rounded} border-2 border-dashed border-slate-300 flex items-center justify-center`}>
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-[#F4F5FA]">
        <div className="text-center bg-white rounded-2xl border border-slate-200 shadow-sm px-10 py-12">
          ...
          <p className="text-slate-600 font-medium">Job category not found</p>
          <button
            onClick={handleBack}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <p className="text-[11px] text-slate-400 leading-tight">Job Categories</p>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            {isViewMode ? (
              <>
                ...
                <button
                  type="button"
                  onClick={() => navigate(`/job-categories/edit/${id}`, { state: { item: data } })}
            ...
            {TABS.map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  {active && (
                    <motion.span
                      layoutId="job-category-tab-underline"
