// // pages/subindustries/SubIndustryForm.jsx
...
//                     if (!item) {
//                         const response = await subIndustryService.getById(id);
//                         item = response.data;
//                     }
...
//                         slug: item.slug || "",
//                         icon: item.icon || null,
//                         industry_id: item.industry_id || item.Industry?.id || "",
...
//                     showError("Failed to load sub-industry data");
//                     navigate('/subindustries');
//                 } finally {
...
//                 viewRender: (value, row) => row?.parent_industry_name || value || '—'
//             },
//             {
//                 name: "icon",
...
//             custom: (value) => {
//                 // We'll check uniqueness in the submit handler
//                 return null;
//             }
//         },
//         industry_id: {
...
//             const allData = await subIndustryService.getAll({ limit: 1000 });
...
//             // if (duplicate) {
//             //     showError('This sub-industry name already exists');
//             //     setLoading(false);
//             //     return;
//             // }
...
//                 parent_industry_name: data.parent_industry_name || "",
//             };
//         }
...
//     const getTitle = () => {
//         if (mode === 'view') return 'Sub-Industry Details';
//         if (mode === 'edit') return 'Edit Sub-Industry';
//         return 'Add New Sub-Industry';
//     };

//     // Get submit label
//     const getSubmitLabel = () => {
//         if (mode === 'edit') return 'Update Sub-Industry';
//         return 'Create Sub-Industry';
//     };
...
//                     <p className="mt-3 text-gray-500">Loading sub-industry data...</p>
//                 </div>
//             </div>
//         );
//     }
...
//             <div className="min-h-screen bg-gray-50 flex items-center justify-center">
//                 <div className="text-center">
//                     <p className="text-gray-500">Sub-industry not found</p>
//                     <button
//                         onClick={() => navigate('/subindustries')}
...
//             navigateTo="/subindustries"
//             submitLabel={getSubmitLabel()}
//             editLabel="Edit Sub-Industry"
//             deleteLabel="Delete Sub-Industry"
//             loading={loading}
//             showDelete={mode !== 'add'}
//             showEdit={mode === 'view'}
//             // enableEditMode={mode === 'view'}
//             breadcrumb={mode === 'view' ? 'Viewing sub-industry details' : mode === 'edit' ? 'Updating sub-industry' : 'Creating new sub-industry'}
//             onEdit={() => navigate(`/subindustries/edit/${id}`, { state: { item: data } })}
//         />
//     );
// };
...
const SubIndustryForm = () => {
    ...
    const fetchData = async () => {
      if ((mode === "edit" || mode === "view") && id) {
        ...
        try {
          ...
          if (!item) throw new Error("Sub-industry not found");
  ...
  const validate = () => {
    ...
    if (!formValues.name?.trim()) {
      ...
      newErrors.name = "Sub-industry name must be at least 2 characters";
    } else if (formValues.name.trim().length > 50) {
      newErrors.name = "Sub-industry name must be at most 50 characters";
    }
    ...
    const sortOrder =
      ...
      formValues.sort_order === null ||
      formValues.sort_order === undefined
        ? 0
        : Number(formValues.sort_order);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const submitData = {
        ...
        industry_id: parseInt(formValues.industry_id),
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-[#F4F5FA]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-slate-400">Loading sub-industry data...</p>
        </div>
      </div>
    );
  }
  ...
  if ((mode === "view" || mode === "edit") && !data && !fetchLoading) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-[#F4F5FA]">
        <div className="text-center">
          <p className="text-slate-500">Sub-industry not found</p>
          <button
            onClick={() => navigate("/subindustries")}
  ...
  // ─── Hero helpers ────────────────────────────────────────────
  const heroName = formValues.name?.trim() || "New Sub-Industry";
          ...
          <div className="space-y-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div className="sm:col-span-2">
                <FieldLabel required>Sub-Industry Name</FieldLabel>
                ...
                {errors.name ? (
                  ...
                  <p className="text-xs text-slate-500 mt-1.5">
                    Enter a unique name for the sub-industry.
                  </p>
                )}
              </div>

              <div className="sm:col-span-2">
                <FieldLabel required>Parent Industry</FieldLabel>
                <div className="relative">
                  <MdCategory
              ...
              {formValues.slug && (
                <div>
                  ...
                  <input
                    ...
                    disabled
                    className="w-full px-4 py-2.5 border border-slate-200 rounded-lg text-sm bg-slate-50 text-slate-700 cursor-not-allowed font-mono"
                  />
                </div>
              )}
            </div>
      ...
      case "media":
        return (
          <div className="grid grid-cols-1 gap-6">
            <div>
              <FieldLabel>Sub-Industry Icon</FieldLabel>
          ...
          <div className="space-y-6 max-w-xl">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-2">
                Active sub-industries are visible in the listing.
              </p>
            </div>
            ...
            <div className="pt-2 border-t border-slate-100">
              ...
              <p className="text-xs text-slate-400 mt-1.5">
                Trending sub-industries are highlighted in the listing.
              </p>
            </div>
          </div>
        );
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            {mode === "view" ? (
              <button
                type="button"
                onClick={() =>
                  navigate(`/subindustries/edit/${id}`, {
                    state: { item: data },
                  })
                }
            ...
            <div className="flex flex-col sm:flex-row sm:items-end gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-2 flex items-center gap-2 flex-wrap">
                  {formValues.parent_industry_name && (
                ...
                {formValues.parent_industry_name ||
                  (formValues.industry_id
                    ? industryOptions.find(
                        (opt) => opt.value === String(formValues.industry_id)
                      )?.label || "—"
                    : "—")}
              </p>
            </div>
            ...
            {TABS.map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  {active && (
                    <motion.span
                      layoutId="subindustry-tab-underline"
              ...
              <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto sm:ml-auto">
                ...
                {mode === "view" ? (
                  ...
                  >
                    ...
                    {loading ? "Saving..." : "Update Sub-Industry"}
                  </button>
                )}
              </div>
            </div>
