// // pages/education/EducationSubCategoryForm.jsx
...
// const EducationSubCategoryForm = () => {
...
//           if (!item) {
//             const response = await educationSubCategoryService.getById(id);
//             item = response.data;
//           }

//           // Normalize the data based on API response
//           const normalizedData = {
//             id: item.id || item._id,
//             name: item.name || "",
//             education_id: item.education_id || "",
//             education_category_name: item.education_category_name || item.education_id?.name || "",
...
//           showError("Failed to load education sub-category data");
//           navigate('/education-subcategories');
//         } finally {
...
//         help: "Select the parent education category",
//         viewRender: (value, row) => row?.education_category_name || value || '—'
//       },
//       {
//         name: "name",
...
//         type: "text",
//         required: true,
//         placeholder: "e.g. B.Voc Beauty & Wellness",
...
//       if (!formData.education_id || formData.education_id === '') {
...
//       const allData = await educationSubCategoryService.getAll({ limit: 1000 });
...
//     try {
//       await educationSubCategoryService.delete(id);
...
//         education_category_name: data.education_category_name || "",
//       };
...
//   const getTitle = () => {
//     if (mode === 'view') return 'Education Sub-Category Details';
//     if (mode === 'edit') return 'Edit Education Sub-Category';
//     return 'Add New Education Sub-Category';
//   };
...
//       <div className="min-h-screen bg-gray-50 flex items-center justify-center">
//         <div className="text-center">
//           <p className="text-gray-500">Education sub-category not found</p>
//           <button
//             onClick={() => navigate('/education-subcategories')}
...
//       navigateTo="/education-subcategories"
...
//       onEdit={() => navigate(`/education-subcategories/edit/${id}`, { state: { item: data } })}
//     />
//   );
// };

// export default EducationSubCategoryForm;
...
const EducationSubCategoryForm = () => {
      ...
      if ((mode === 'edit' || mode === 'view') && id) {
        ...
        try {
          ...
          setFormValues({
            ...
            education_id: normalizedData.education_id || '',
  ...
  const validateField = (name, value) => {
    switch (name) {
      case 'name':
        if (!value || !value.trim()) return 'Sub-category name is required';
        if (value.trim().length < 2) return 'Sub-category name must be at least 2 characters';
        if (value.trim().length > 50) return 'Sub-category name must be at most 50 characters';
        return null;
      case 'education_id':
  ...
  const validateForm = () => {
    ...
    const parentError = validateField('education_id', formValues.education_id);
    if (parentError) {
      newErrors.education_id = parentError;
      isValid = false;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const submitData = {
        ...
        education_id: parseInt(formValues.education_id),
  ...
  const handleBack = () => navigate('/education-subcategories');
  ...
  const renderField = (field) => {
    ...
    if (isViewMode) {
      ...
      if (name === 'education_id') {
        displayValue = data?.education_category_name || value || '—';
      } else if (name === 'status') {
  ...
  const subCategoryName = formValues.name?.trim() || 'New Sub-Category';
  const status = formValues.status || 'active';
  const isTrending = formValues.is_trending || false;
  const parentName = data?.education_category_name ||
    parentCategories.find((c) => String(c.id || c._id) === String(formValues.education_id))?.name ||
    '—';
  const createdDate = data?.created_at ? formatDate(data.created_at) : '—';
  const updatedDate = data?.updated_at ? formatDate(data.updated_at) : '—';

  const initials = subCategoryName
    .split(' ')
    .filter(Boolean)
    .slice(0, 2)
  ...
  <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-4">
    ...
    <div className="flex items-center gap-3 min-w-0">
      <button
        type="button"
        onClick={handleBack}
        className="p-2 rounded-lg hover:bg-slate-100 active:bg-slate-200 transition-colors flex-shrink-0"
        aria-label="Back"
      >
        <MdArrowBack size={19} className="text-slate-500" />
      </button>
      ...
      <div className="min-w-0">
        ...
        <p className="text-sm font-semibold text-slate-700 truncate leading-tight max-w-[45vw]">
          {isViewMode
            ? perkName
            : isEditMode
              ? `Edit: ${perkName}`
    ...
    <div className="flex items-center gap-2 flex-shrink-0">
      {isViewMode ? (
        <>
          ...
          {mode !== 'add' && (
            <button
              ...
              className="p-2 rounded-lg text-red-500 hover:bg-red-50 hover:text-red-600 active:bg-red-100 transition-colors"
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-4 py-2 bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white text-sm font-semibold rounded-lg shadow-sm shadow-blue-600/20 transition-colors"
          >
            <MdSave size={16} />
            Edit Perk Benefit
          </button>
        ...
        <>
          ...
          <button
            type="button"
            onClick={handleBack}
            className="hidden sm:inline-flex items-center gap-1.5 px-3.5 py-2 bg-slate-100 hover:bg-slate-200 active:bg-slate-300 text-slate-600 text-sm font-medium rounded-lg border border-slate-200 transition-colors"
          >
            <MdCancel size={16} />
            Cancel
          </button>
          ...
          <button
            ...
            disabled={loading}
            className="inline-flex items-center gap-1.5 px-4 py-2 bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white text-sm font-semibold rounded-lg shadow-sm shadow-blue-600/20 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loading ? (
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-6">
        ...
        >
          <div className="relative h-44 sm:h-52 bg-gradient-to-br from-purple-900 via-indigo-900 to-slate-950">
            <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-slate-950/50 to-slate-900/10" />
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-2 flex items-center gap-2 flex-wrap">
                  <span className="text-xs text-white/70 flex items-center gap-1">
                    <MdFolder size={12} /> Parent: {parentName}
                  </span>
                  {isViewMode && (
                    <>
                  ...
                  {isAddMode && (
                    <span className="text-xs text-white/50">New Sub-Category</span>
                  )}
                </div>
              </div>
            </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4">
          ...
          <div className="flex items-center gap-2.5 rounded-xl bg-white/80 backdrop-blur-sm px-3.5 py-2.5 border border-slate-200 shadow-sm">
            <MdFolder size={16} className="text-slate-400 flex-shrink-0" />
            <div className="min-w-0">
              <p className="text-[10px] text-slate-500 leading-tight">Parent Category</p>
              <p className="text-sm font-semibold text-slate-700 truncate">
                {parentName}
              </p>
            </div>
            ...
            {TABS.map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  {active && (
                    <motion.span
                      layoutId="education-subcategory-tab-underline"
      ...
      <ConfirmDialog
        ...
        message={`Delete sub-category "${subCategoryName}"? This action cannot be undone.`}
      />
    </div>
  );
};
