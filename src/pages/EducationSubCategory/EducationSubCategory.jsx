...
//       showError(error.message || "Failed to load education sub-categories");
//     } finally {
//       setLoading(false);
//     }
//   };
...
//         String(item.education_category_name ?? "").toLowerCase().includes(query)
//       );
//     }
//     return result;
...
//       if (editItem) {
//         await educationSubCategoryService.update(editItem.id, submitData);
...
//     try {
//       await educationSubCategoryService.delete(deleteId);
...
//       await educationSubCategoryService.update(row.id, updateData);
...
//         <div>
//           <h1 className="text-2xl font-bold text-gray-900">Education Sub-Categories</h1>
//           <p className="text-sm text-gray-500 mt-1">Manage education sub-categories for job listings</p>
//         </div>
...
//       {/* Form Modal - Fix initialData for education_id */}
//       <FormModal
//         isOpen={modalOpen}
...
//           name: editItem.name || "",
//           // IMPORTANT: Get the education_id from the edit item
//           // Check both direct and nested values
//           education_id: editItem.education_id || (editItem.raw?.education_id) || "",
...
//             <ViewRow label="Parent Category" value={viewData.education_category_name || "-"} />
//             <ViewRow
//               label="Status"
//               value={
...
// pages/EducationSubCategory.jsx
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (id, row) => (
        <div className="flex gap-1">
          <button
            onClick={() => navigate(`/education-subcategories/view/${row.id}`, { state: { item: row } })}
            className="p-1.5 hover:bg-blue-50 text-[#2c0eee] rounded-lg transition-colors"
            title="View"
          >
            <MdVisibility size={16} />
          ...
          <button
            onClick={() => navigate(`/education-subcategories/edit/${row.id}`, { state: { item: row } })}
            className="p-1.5 hover:bg-gray-100 text-gray-500 rounded-lg transition-colors"
            title="Edit"
          >
            <MdEdit size={16} />
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        ...
        <Button icon={MdAdd} onClick={() => navigate('/education-subcategories/add')}>
          Add Sub-Category
        </Button>
      </div>
