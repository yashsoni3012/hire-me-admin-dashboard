...
const EditProjectSettings = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Boolean values may come back as 1/0 or true/false — normalize
        // to "true"/"false" strings for the select in the Value tab.
        if (normalized.value_type === "boolean") {
          normalized.setting_value = toBool(item.setting_value)
            ? "true"
            : "false";
        }
        ...
        // Capture existing file URL for the File value_type
        if (normalized.value_type === "file" && normalized.setting_value) {
          setExistingFile(getFileUrl(normalized.setting_value));
        }
      } catch (error) {
        console.error("Fetch error:", error);
        showError("Failed to load project setting data");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // If value_type is file and a new file was chosen, send it.
      // If no new file was chosen, submitData.setting_value already
      // carries forward the existing path.
      if (submitData.value_type === "file" && selectedFile) {
        payload.setting_file = selectedFile;
      }
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-[#F4F5FA]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-slate-400">Loading setting data...</p>
        </div>
      </div>
    );
  }
  ...
  // ─── Hero helpers ────────────────────────────────────────────
  const heroKey = formValues.setting_key?.trim() || "Setting";
  ...
  const displayedFile = filePreview || existingFile;
        ...
        <div className="flex flex-col sm:flex-row items-start gap-4">
          ...
          <div className="flex flex-col gap-2">
            <label className="inline-flex items-center gap-2 px-3.5 py-2 bg-blue-50 hover:bg-blue-100 text-blue-700 text-sm font-medium rounded-lg cursor-pointer transition-colors">
              ...
              {displayedFile ? "Change file" : "Upload file"}
              <input
                type="file"
            ...
            <p className="text-xs text-slate-500">
              Max 5MB.
              {existingFile && !filePreview && " Current file shown."}
            </p>
          </div>
        </div>
      );
            ...
            {TABS.map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  {active && (
                    <motion.span
                      layoutId="edit-project-setting-tab-underline"
