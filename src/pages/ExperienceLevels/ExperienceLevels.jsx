// pages/experience-levels/ExperienceLevels.jsx
...
const ExperienceLevels = () => {
  ...
  // Normalize experience level data
  const normalizeExperienceLevel = (item) => {
    return {
      id: item.id || item._id,
      name: item.name || "",
      min_year: item.min_year || 0,
      max_year: item.max_year || 1,
  ...
  const load = async () => {
    ...
    try {
      ...
      const r = await experienceLevelService.getAll({ limit: 1000 });
      const rawData = r.data?.data || r.data?.results || r.data || [];
      const experienceLevels = Array.isArray(rawData) ? rawData.map(normalizeExperienceLevel) : [];

      const sortedExperienceLevels = experienceLevels.sort((a, b) => {
        return new Date(b.created_at) - new Date(a.created_at);
      });

      setData(sortedExperienceLevels);
    } catch (error) {
      console.error("Load error:", error);
      showError(error.message || "Failed to load experience levels");
    } finally {
      setLoading(false);
    }
  };
  ...
  // Navigation handlers
  const openAdd = () => {
    navigate('/experience-levels/add');
  };

  const openEdit = (item) => {
    navigate(`/experience-levels/edit/${item.id}`);
  };

  const openView = (item) => {
    navigate(`/experience-levels/view/${item.id}`);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await experienceLevelService.delete(deleteId);
  ...
  // ─── FIXED: Toggle status - properly updates the data ──────────
  const handleStatusToggle = async (row) => {
    ...
    try {
      const updateData = {
        ...
        is_status: newStatus,  // Use is_status consistently
        status: newStatus,     // Also send status for compatibility
      };

      console.log('Updating status:', updateData);
      await experienceLevelService.update(row.id, updateData);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Experience Levels</h1>
          <p className="text-sm text-gray-500 mt-1">
            Manage experience levels for job listings
          </p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-5 py-3 border-t border-gray-100">
          <p className="text-xs text-gray-400">
            ...
            {filteredData.length} experience levels
          </p>
      ...
      <ConfirmDialog
        ...
        message="Delete this experience level? This action cannot be undone."
      />
    </div>
  );
};
