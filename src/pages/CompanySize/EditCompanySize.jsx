...
//         if (data) {
//           const formData = {
//             company_size_name: data.name || data.company_size_name || "",
//             company_size_slug: data.slug || data.company_size_slug || "",
...
//       showSuccess("Company size updated successfully");
//       navigate('/company-sizes');
//     } catch (error) {
//       console.error('Submit error:', error);
//       showError(error.message || "Failed to update company size");
//     } finally {
//       setLoading(false);
//     }
//   };
...
//       showError(error.message || "Failed to delete company size");
//     } finally {
...
//           <p className="text-sm text-gray-400">Loading company size data...</p>
//         </div>
//       </div>
//     );
//   }
...
//       breadcrumb={`Editing: ${editItem?.name || 'Company Size'}`}
//       showEdit={false}
//     />
//   );
// };

// export default EditCompanySize;

// pages/company-sizes/EditCompanySize.jsx
import React, { useState, useEffect } from "react";
import { useNavigate, useParams } from "react-router-dom";
...
const EditCompanySize = () => {
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-[#F4F5FA]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-slate-400">Loading company size data...</p>
        </div>
      </div>
    );
  }
  ...
  // ─── Hero helpers ──────────────────────────────────────────────
  const heroName = formValues.company_size_name?.trim() || "Company Size";
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            >
              ...
              {loading ? "Updating..." : "Update Company Size"}
            </button>
          </div>
        </div>
      </div>
            ...
            {TABS.map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  {active && (
                    <motion.span
                      layoutId="edit-company-size-tab-underline"
