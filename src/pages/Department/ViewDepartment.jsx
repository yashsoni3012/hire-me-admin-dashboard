...
//           // Determine active status - check multiple possible fields
//           let isActive = false;
...
//           } else if (data.isStatus !== undefined && data.isStatus !== null) {
//             isActive = data.isStatus === true || data.isStatus === 1 || data.isStatus === "1" || data.isStatus === "true";
//           }
...
//       readonly: true,
//       viewRender: (value) => {
//         if (!value) return <span className="text-gray-400 text-sm">No icon</span>;
...
//           <p className="text-sm text-gray-400">Loading department details...</p>
//         </div>
//       </div>
//     );
//   }
...
//       breadcrumb={`Viewing: ${viewData?.department_name || viewData?.name || 'Department'}`}
//       enableEditMode={true}
//       showEdit={true}
//       editLabel="Edit Department"
//       cancelLabel="Back to Departments"
//     />
//   );
// };
...
// pages/departments/ViewDepartment.jsx
import React, { useState, useEffect } from 'react';
import { useNavigate, useParams } from 'react-router-dom';
...
const ViewDepartment = () => {
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-[#F4F5FA]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-slate-400">Loading department details...</p>
        </div>
      </div>
    );
  }
  ...
  // ─── Compute hero data ────────────────────────────────────
  const departmentName = departmentData.department_name || 'Department';
  const status = departmentData.status || 'inactive';
  const isTrending = departmentData.is_trending || false;
  const iconUrl = getFullImageUrl(departmentData.icon);
  const createdDate = departmentData.created_at ? formatDate(departmentData.created_at) : '—';
  const updatedDate = departmentData.updated_at ? formatDate(departmentData.updated_at) : '—';
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div>
                ...
                <ReadOnlyValue>
                  <span className="font-medium text-lg">{departmentName}</span>
                </ReadOnlyValue>
              </div>
              <div>
              ...
              <div>
                <ReadOnlyValue>#{departmentData.id}</ReadOnlyValue>
              </div>
              <div className="sm:col-span-2">
                <FieldLabel>Department Icon</FieldLabel>
                <ReadOnlyValue>
                  {iconUrl ? (
                    <div className="relative group inline-block">
                      <img
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
              ...
              <div className="p-6 space-y-5">
                <div>
                  ...
                  <ReadOnlyValue>
                    {departmentData.created_by ? getUserNameCached(departmentData.created_by) : '—'}
                  </ReadOnlyValue>
                </div>
                <div>
                ...
                <div>
                  ...
                  <ReadOnlyValue>
                    {departmentData.updated_by ? getUserNameCached(departmentData.updated_by) : '—'}
                  </ReadOnlyValue>
                </div>
                <div>
            ...
            {TABS.map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  {active && (
                    <motion.span
                      layoutId="view-department-tab-underline"
