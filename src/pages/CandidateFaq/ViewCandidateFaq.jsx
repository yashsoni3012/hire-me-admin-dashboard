...
//   const handleEdit = () => {
//     navigate(`/candidate-faq/edit/${id}`);
//   };
...
//       breadcrumb={`Viewing: ${viewData?.question || 'FAQ'}`}
//       enableEditMode={true}
//       showEdit={true}
//       editLabel="Edit FAQ"
//       cancelLabel="Back to FAQs"
//     />
//   );
// };

// export default ViewCandidateFaq;

// pages/candidate-faq/ViewCandidateFaq.jsx
import React, { useState, useEffect } from 'react';
import { useNavigate, useParams } from 'react-router-dom';
...
const ViewCandidateFaq = () => {
  ...
  const handleBack = () => navigate('/candidate-faq');
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-[#F4F5FA]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-slate-400">Loading FAQ details...</p>
        </div>
      </div>
    );
  }
  ...
  // ─── Compute hero data ────────────────────────────────────
  const question = faqData.question || 'FAQ';
  const answer = faqData.answer || '';
  const order = faqData.order ?? 0;
  const status = faqData.status || 'inactive';
  const isTrending = faqData.is_trending || false;
  const createdDate = faqData.created_at ? formatDate(faqData.created_at) : '—';
  const updatedDate = faqData.updated_at ? formatDate(faqData.updated_at) : '—';
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
              ...
              <div className="p-6 space-y-5">
                <div>
                  ...
                  <ReadOnlyValue>
                    {faqData.created_by ? getUserNameCached(faqData.created_by) : '—'}
                  </ReadOnlyValue>
                </div>
                <div>
                ...
                <div>
                  ...
                  <ReadOnlyValue>
                    {faqData.updated_by ? getUserNameCached(faqData.updated_by) : '—'}
                  </ReadOnlyValue>
                </div>
                <div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <p className="text-[11px] text-slate-400 leading-tight">Candidate FAQs</p>
            ...
            <div className="flex flex-col sm:flex-row sm:items-end gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-2 flex items-center gap-2 flex-wrap">
                  ...
                  <span className="text-xs text-white/70">ID: #{faqData.id}</span>
                  {faqData.created_at && (
                    <>
                      <span className="text-xs text-white/70">•</span>
            ...
            {TABS.map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  {active && (
                    <motion.span
                      layoutId="view-candidate-faq-tab-underline"
