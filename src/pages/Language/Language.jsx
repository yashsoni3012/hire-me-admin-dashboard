...
const Language = () => {
  ...
  const normalizeLanguage = (item) => ({
    id: item.id || item._id,
    language_name: item.language_name || item.name || "",
    is_trending: item.is_trending === true || item.is_trending === 1 || item.is_trending === "1",
    is_status: item.status === true || item.status === 1 || item.status === "1" || item.is_status === true || item.is_status === 1,
  ...
  const loadLanguages = async () => {
    ...
    try {
      ...
      const r = await languageService.getAll({ limit: 1000 });
      const rawData = r.data?.data || r.data?.results || r.data || [];
      const languages = Array.isArray(rawData) ? rawData.map(normalizeLanguage) : [];
      ...
      showError(error.message || "Failed to load languages");
    } finally {
      setLoading(false);
    }
  };
  ...
  const openEdit = (item) => {
    navigate(`/languages/edit/${item.id}`);
  };

  const openView = (item) => {
    navigate(`/languages/view/${item.id}`);
  };
  ...
  const handleStatusToggle = async (row) => {
    ...
    try {
      ...
      await languageService.update(row.id, updateData);
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "language_name",
      render: (v) => <span className="font-medium capitalize text-gray-800">{v}</span>,
    },
    {
      header: "Trending",
      key: "is_trending",
      render: (value, row) => (
        <button
          ...
          className={`relative inline-flex items-center h-6 rounded-full w-11 transition-colors focus:outline-none ${value === true ? "bg-yellow-500" : "bg-gray-300"}`}
        >
          <span
            className={`inline-block w-4 h-4 transform bg-white rounded-full transition-transform shadow ${value === true ? "translate-x-6" : "translate-x-1"}`}
          />
        </button>
      ),
    },
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Manage languages for job listings</p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <Table columns={columns} data={paginatedData} loading={loading} emptyMessage="No languages found" />
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-5 py-3 border-t border-gray-100">
          <p className="text-xs text-gray-400">
            ...
            {"–"}
            {Math.min(page * limit, filteredData.length)} of {filteredData.length} languages
          </p>
      ...
      <ConfirmDialog
        ...
        message="Delete this language? This action cannot be undone."
      />
    </div>
  );
};
