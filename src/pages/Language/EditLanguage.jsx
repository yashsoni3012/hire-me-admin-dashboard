...
//         if (data) {
//           const formData = {
//             language_name: data.language_name || data.name || "",
...
//         showError(error.message || "Failed to load language data");
//         navigate('/languages');
//       } finally {
...
//       help: "Enter the full name of the language",
//     },
//     {
//       name: "status",
//       label: "Status",
...
//       help: "Trending languages will be highlighted in the listing",
//     },
//   ];
...
//     language_name: {
//       required: true,
//       requiredMessage: "Language name is required",
//       minLength: 2,
//       minLengthMessage: "Language name must be at least 2 characters",
//       maxLength: 100,
//       maxLengthMessage: "Language name must be at most 100 characters",
//     },
//   };
...
//       await languageService.update(id, submitData);
//       showSuccess("Language updated successfully");
      
//       // Redirect to languages list page
//       navigate('/languages');
//     } catch (error) {
//       console.error('Submit error:', error);
//       showError(error.message || error?.response?.data?.message || "Failed to update language");
//     } finally {
//       setLoading(false);
//     }
//   };
...
//         showError("Cannot delete this language because it is being used in other records.");
//       } else {
//         showError(message || "Failed to delete language");
//       }
//     } finally {
...
//           <p className="text-sm text-gray-400">Loading language data...</p>
//         </div>
//       </div>
//     );
//   }
...
//       breadcrumb={`Editing: ${editItem?.language_name || 'Language'}`}
//       showEdit={false}
//     />
//   );
// };
...
// pages/languages/EditLanguage.jsx
import React, { useState, useEffect } from 'react';
import { useNavigate, useParams } from 'react-router-dom';
...
const EditLanguage = () => {
  ...
  const validateField = (name, value) => {
    if (name === 'language_name') {
      if (!value || !value.trim()) return 'Language name is required';
      if (value.trim().length < 2) return 'Language name must be at least 2 characters';
      if (value.trim().length > 100) return 'Language name must be at most 100 characters';
    }
    return null;
  };
  ...
  const validateForm = () => {
    ...
    const error = validateField('language_name', formValues.language_name);
    if (error) {
      newErrors.language_name = error;
      isValid = false;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const submitData = {
        language_name: formValues.language_name.trim(),
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-[#F4F5FA]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-slate-400">Loading language data...</p>
        </div>
      </div>
    );
  }
  ...
  // ─── Compute hero data ────────────────────────────────────
  const languageName = formValues.language_name?.trim() || 'Language';
  ...
  return (
    <div className="min-h-screen pb-16 bg-[#F4F5FA]">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <p className="text-[11px] text-slate-400 leading-tight">Languages</p>
            ...
            {TABS.map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  {active && (
                    <motion.span
                      layoutId="edit-language-tab-underline"
      ...
      <ConfirmDialog
        ...
        title="Delete Language"
        message={`Delete language "${languageName}"? This action cannot be undone.`}
      />
    </div>
  );
};
