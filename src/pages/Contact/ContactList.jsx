...
const ContactList = () => {
    ...
    // ─── FIXED: Filter data with proper status filtering ────────
    const filteredData = useMemo(() => {
        let result = data;
    ...
    // ─── FIXED: Calculate status counts correctly ────────────────
    ...
    const handleAssignTask = async (contactId, userId) => {
        try {
            await contactService.assign(contactId, userId);
            const user = getUserById(userId);
            const userName = user?.name || user?.username || user?.email || `Admin ${userId}`;
            ...
            console.error("Assign error:", error);
            showError(error?.response?.data?.message || error?.message || "Failed to assign task");
        }
    };
    ...
    const handleUnassignTask = async (contactId) => {
        try {
            ...
            console.error("Unassign error:", error);
            showError(error?.response?.data?.message || error?.message || "Failed to unassign task");
        }
    };
            ...
            <div ref={wrapperRef} className="relative inline-block">
                ...
                >
                    {currentAssignedTo ? (
                        ...
                        <>
                            <MdPersonAdd className="text-gray-400 group-hover:text-purple-500" size={16} />
                ...
                {isOpen && createPortal(
                    <div
                        className="assignment-dropdown-portal fixed w-64 bg-white rounded-xl border border-gray-200 shadow-2xl z-[99999] overflow-hidden"
                        style={{
                            top: dropdownPosition.top,
                            left: dropdownPosition.left,
                        }}
                    >
                        ...
                        <div className="max-h-60 overflow-y-auto py-1">
                            {users.length === 0 ? (
                                <div className="px-4 py-4 text-sm text-gray-400 text-center">No users available</div>
                            ) : (
                                users.map((user) => {
                                    const userId = user.id ?? user._id;
                                    const selected = getId(currentAssignedTo) === getId(userId);
                                    return (
                                        <button
                                            key={String(userId)}
                                            type="button"
                                        ...
                                        >
                                            ...
                                            {selected && <MdCheck className="text-purple-600 flex-shrink-0" size={18} />}
                                        </button>
                                    );
                                })
                            )}
                        ...
                        <div className="border-t border-gray-100 bg-gray-50/50 px-4 py-2">
                            {currentAssignedTo ? (
                                ...
                                <div className="text-xs text-gray-400 text-center py-1">Select a user to assign this task</div>
                            )}
                        </div>
                    </div>,
                    document.body
