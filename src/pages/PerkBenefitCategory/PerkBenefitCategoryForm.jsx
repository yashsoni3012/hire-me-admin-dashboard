// // pages/perks/PerkBenefitCategoryForm.jsx
...
//     const [perkBenefits, setPerkBenefits] = useState([]);
...
//     // Fetch perk benefits to check if category has children
//     useEffect(() => {
//         const loadPerkBenefits = async () => {
//             try {
...
//                 setPerkBenefits(benefits);
//             } catch (error) {
//                 console.error('Failed to load perk benefits:', error);
//             }
//         };
//         loadPerkBenefits();
//     }, []);
...
//                     if (!item) {
//                         const response = await perkBenefitCategoryService.getById(id);
//                         item = response.data;
//                     }
...
//                     showError("Failed to load perk benefit category data");
//                     navigate('/perk-benefit-categories');
//                 } finally {
...
//     // Check if category has perk benefits
//     const getPerkBenefitsCount = (categoryId) => {
//         return perkBenefits.filter((benefit) => {
//             const parentId = benefit.category_id || benefit.perks_benefits_category?.id;
...
//                 help: "Enter a unique name for the perk benefit category",
...
//             const allData = await perkBenefitCategoryService.getAll({ limit: 1000 });
...
//         // Check if category has perk benefits
//         const childCount = getPerkBenefitsCount(id);
//         if (childCount > 0) {
//             showError(`Cannot delete this category because it has ${childCount} perk benefit${childCount === 1 ? "" : "s"} linked to it.`);
//             throw new Error("Category has perk benefits");
//         }

//         try {
//             await perkBenefitCategoryService.delete(id);
//             showSuccess("Category deleted successfully");
//             navigate('/perk-benefit-categories');
//         } catch (error) {
//             console.error('Delete error:', error);
//             const message = error?.response?.data?.message || error?.message || "";
//             if (/perk[-_ ]?benefit|child|foreign\s*key|constraint/i.test(message)) {
//                 showError("Cannot delete this category because it has perk benefits linked to it.");
//             } else {
...
//     const getTitle = () => {
//         if (mode === 'view') return 'Perk & Benefit Category Details';
//         if (mode === 'edit') return 'Edit Perk & Benefit Category';
//         return 'Add New Perk & Benefit Category';
//     };
...
//             <div className="min-h-screen bg-gray-50 flex items-center justify-center">
//                 <div className="text-center">
//                     <p className="text-gray-500">Perk benefit category not found</p>
//                     <button
//                         onClick={() => navigate('/perk-benefit-categories')}
...
//             navigateTo="/perk-benefit-categories"
...
//             onEdit={() => navigate(`/perk-benefit-categories/edit/${id}`, { state: { item: data } })}
//         />
//     );
// };

// export default PerkBenefitCategoryForm;
...
const PerkBenefitCategoryForm = () => {
  ...
  // ─── Helper: check if category has perk benefits ─────────────
  const getPerkBenefitsCount = (categoryId) => {
    return perkBenefits.filter((benefit) => {
      const parentId = benefit.category_id || benefit.perks_benefits_category?.id;
      return String(parentId) === String(categoryId);
  ...
  const handleDelete = async () => {
    ...
    if (childCount > 0) {
      showError(
        `Cannot delete this category because it has ${childCount} perk benefit${childCount === 1 ? '' : 's'} linked to it.`
      );
  ...
  const handleBack = () => navigate('/perk-benefit-categories');
  ...
  const perkCount = data?.id ? getPerkBenefitsCount(data.id) : 0;
  ...
  return (
    <div className="min-h-screen pb-16 ">
      ...
      <div className="backdrop-blur-md border-b border-slate-700/50 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            <button
              ...
              aria-label="Back"
            >
              <MdArrowBack size={19} className="text-slate-400" />
            </button>
            <div className="min-w-0">
              <p className="text-[11px] text-slate-500 leading-tight">Perk & Benefit Categories</p>
              <p className="text-sm font-semibold text-black truncate leading-tight max-w-[45vw]">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            {isViewMode ? (
              <>
                ...
                <button
                  type="button"
                  onClick={() => navigate(`/perk-benefit-categories/edit/${id}`, { state: { item: data } })}
              ...
              <>
                ...
                <button
                  type="button"
                  onClick={handleBack}
                  className="hidden sm:inline-flex items-center gap-1.5 px-3.5 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 text-sm font-medium rounded-lg transition-colors"
                >
                  <MdCancel size={16} />
                  Cancel
                </button>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-6">
                ...
                <div className="mt-2 flex items-center gap-2 flex-wrap">
                  {perkCount > 0 && (
                    <span className="text-xs text-slate-300">
                      {perkCount} perk benefit{perkCount === 1 ? '' : 's'}
                    </span>
                  )}
                  {isViewMode && (
                    <>
                  ...
                  {isAddMode && (
                    <span className="text-xs text-slate-400">New Category</span>
                  )}
                </div>
              </div>
            </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4">
          ...
          <div className="flex items-center gap-2.5 rounded-xl bg-white/80 backdrop-blur-sm px-3.5 py-2.5 border border-slate-200 shadow-sm">
            <MdLocalOffer size={16} className="text-slate-400 flex-shrink-0" />
            <div className="min-w-0">
              <p className="text-[10px] text-slate-500 leading-tight">Perk Benefits</p>
              <p className="text-sm font-semibold text-slate-700 truncate">
                {perkCount}
              </p>
            </div>
            ...
            {TABS.map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  {active && (
                    <motion.span
                      layoutId="perk-category-tab-underline"
      ...
      <ConfirmDialog
        ...
        title="Delete Category"
        message={`Delete category "${categoryName}"? This action cannot be undone.${perkCount > 0 ? ` It has ${perkCount} perk benefit${perkCount === 1 ? '' : 's'} linked.` : ''}`}
      />
    </div>
  );
};
