...
// //       showError(error.message || "Failed to load perk benefit categories");
// //     } finally {
// //       setLoading(false);
// //     }
// //   };
...
// //       if (editItem) {
// //         await perkBenefitCategoryService.update(editItem.id, submitData);
...
// //   //     try {
// //   //       const response = await perkBenefitService.getByCategoryId(deleteId);
// //   //       const subCategories = response.data || response.results || [];

// //   //       if (subCategories.length > 0) {
// //   //         showInfo(
// //   //           `Cannot delete this category because it has ${subCategories.length} sub-categor${subCategories.length > 1 ? 'ies' : 'y'}. Please delete the sub-categories first.`
// //   //         );
...
// //   //       }
// //   //     } catch (error) {
// //   //       console.warn('Could not check sub-categories:', error);
// //   //     }

// //   //     await perkBenefitCategoryService.delete(deleteId);
...
// //     try {
// //       // Check if category has sub-categories
// //       const response = await perkBenefitService.getByCategoryId(categoryId);
// //       const subCategories = response.data || response.results || [];
...
// //       // If we can't check sub-categories, allow deletion
// //       setDeleteId(categoryId);
// //     } finally {
// //       setCheckingSubCategories(prev => ({ ...prev, [categoryId]: false }));
// //     }
...
// //       await perkBenefitCategoryService.update(row.id, updateData);
...
// //         <div>
// //           <h1 className="text-2xl font-bold text-gray-900">Perk & Benefit Categories</h1>
// //           <p className="text-sm text-gray-500 mt-1">Manage perk and benefit categories for job listings</p>
// //         </div>
...
// //         message="Delete this category? Associated perk benefits may be affected."
// //       />
// //     </div>
// //   );
// // };

// // export default PerkBenefitCategory;
...
//   // Fetch all perk benefits so we know which categories have children.
...
//       return benefits;
//     } catch (error) {
//       console.error('Load perk benefits error:', error);
//       return perkBenefits;
//     }
//   };
...
//   // How many perk benefits point back to this category?
...
//   // Fired from the trash icon - blocks the action up front if perk benefits
//   // are still linked, instead of opening a confirm dialog that can't succeed.
//   const handleDeleteClick = (row) => {
//     const childCount = getPerkBenefitsCount(row.id);
//     if (childCount > 0) {
//       showError(
//         `Cannot delete "${row.name}" - it has ${childCount} perk benefit${childCount === 1 ? "" : "s"} linked to it. Please delete or reassign ${childCount === 1 ? "it" : "them"} first.`
//       );
//       return;
//     }
//     setDeleteId(row.id);
...
//   // already verified there are no linked perk benefits, but the catch block
...
//                 }`}
//               title={canDelete ? "Delete" : `Cannot delete - ${childCount} perk benefit${childCount === 1 ? "" : "s"} linked`}
//             >
//               <MdDelete size={16} />
//             </button>
//           </div>
...
const PerkBenefitCategory = () => {
  ...
  const handleDeleteClick = (row) => {
    ...
    if (childCount > 0) {
      showError(
        `Cannot delete it has perk benefit linked to it.`
      );
      return;
    }
    setDeleteId(row.id);
    ...
    },
    {
      ...
      render: (id, row) => {
        ...
        return (
          <div className="flex gap-1">
            <button
              onClick={() => navigate(`/perk-benefit-categories/view/${row.id}`, { state: { item: row } })}
              className="p-1.5 hover:bg-blue-50 text-[#2c0eee] rounded-lg transition-colors"
              title="View"
            >
              <MdVisibility size={16} />
            ...
            <button
              onClick={() => navigate(`/perk-benefit-categories/edit/${row.id}`, { state: { item: row } })}
              className="p-1.5 hover:bg-gray-100 text-gray-500 rounded-lg transition-colors"
              title="Edit"
            >
              <MdEdit size={16} />
            ...
            <button
              ...
              title={canDelete ? "Delete" : `Cannot delete perk benefit linked`}
            >
              <MdDelete size={16} />
            </button>
          </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        ...
        <Button icon={MdAdd} onClick={() => navigate('/perk-benefit-categories/add')}>
          Add Category
        </Button>
      </div>
