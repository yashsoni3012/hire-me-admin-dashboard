// pages/candidate-testimonials/CandidateTestimonials.jsx
...
const CandidateTestimonials = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      const r = await candidateTestimonialService.getAll({ limit: 1000 });
      ...
      showError(error.message || "Failed to load candidate testimonials");
    } finally {
      setLoading(false);
    }
  };
  ...
  // Navigation handlers
  const openAdd = () => {
    navigate("/candidate-testimonials/add");
  };

  const openEdit = (item) => {
    navigate(`/candidate-testimonials/edit/${item.id}`);
  };

  const openView = (item) => {
    navigate(`/candidate-testimonials/view/${item.id}`);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await candidateTestimonialService.delete(deleteId);
  ...
  const handleStatusToggle = async (row) => {
    ...
    try {
      ...
      await candidateTestimonialService.update(row.id, updateData);
      showSuccess(
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Manage testimonials from candidates
          </p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <Table
          ...
          data={paginatedData}
          loading={loading}
          emptyMessage="No candidate testimonials found"
        />
