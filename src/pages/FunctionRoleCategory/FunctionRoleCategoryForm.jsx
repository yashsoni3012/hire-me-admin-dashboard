// // pages/functions/FunctionRoleCategoryForm.jsx
...
// const FunctionRoleCategoryForm = () => {
...
//     // Fetch function roles to check if category has children
//     useEffect(() => {
...
//             } catch (error) {
//                 console.error('Failed to load function roles:', error);
//             }
//         };
//         loadFunctionRoles();
//     }, []);
...
//                     if (!item) {
//                         const response = await functionRoleCategoryService.getById(id);
//                         item = response.data;
//                     }
...
//                     showError("Failed to load function role category data");
//                     navigate('/function-role-categories');
//                 } finally {
...
//     // Check if category has function roles
//     const getFunctionRolesCount = (categoryId) => {
//         return functionRoles.filter((role) => {
//             const parentId = role.roles_category_id || role.functionRoleCategory?.id;
...
//                 help: "Enter a unique name for the function role category",
...
//             const allData = await functionRoleCategoryService.getAll({ limit: 1000 });
...
//         // Check if category has function roles
//         const childCount = getFunctionRolesCount(id);
//         if (childCount > 0) {
//             showError(`Cannot delete this category because it has ${childCount} function role${childCount === 1 ? "" : "s"} linked to it.`);
//             throw new Error("Category has function roles");
//         }

//         try {
//             await functionRoleCategoryService.delete(id);
//             showSuccess("Category deleted successfully");
//             navigate('/function-role-categories');
//         } catch (error) {
//             console.error('Delete error:', error);
//             const message = error?.response?.data?.message || error?.message || "";
//             if (/function[-_ ]?role|child|foreign\s*key|constraint/i.test(message)) {
//                 showError("Cannot delete this category because it has function roles linked to it.");
//             } else {
...
//     const getTitle = () => {
//         if (mode === 'view') return 'Function Role Category Details';
//         if (mode === 'edit') return 'Edit Function Role Category';
//         return 'Add New Function Role Category';
//     };
...
//             <div className="min-h-screen bg-gray-50 flex items-center justify-center">
//                 <div className="text-center">
//                     <p className="text-gray-500">Function role category not found</p>
//                     <button
//                         onClick={() => navigate('/function-role-categories')}
...
//             navigateTo="/function-role-categories"
...
//             onEdit={() => navigate(`/function-role-categories/edit/${id}`, { state: { item: data } })}
//         />
//     );
// };

// export default FunctionRoleCategoryForm;
...
const FunctionRoleCategoryForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const isActive =
        String(formValues.status).toLowerCase() === 'active' ||
  ...
  const handleDelete = async () => {
    ...
    if (childCount > 0) {
      showError(
        `Cannot delete this category because it has ${childCount} function role${childCount === 1 ? '' : 's'} linked to it.`,
      );
  ...
  const handleBack = () => navigate('/function-role-categories');
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-[#F4F5FA]">
        <div className="text-center bg-white rounded-2xl border border-slate-200 shadow-sm px-10 py-12">
          ...
          <p className="text-slate-600 font-medium">Function role category not found</p>
          <button
            onClick={handleBack}
  ...
  const rolesCount = data?.id ? getFunctionRolesCount(data.id) : 0;
  ...
  return (
    <div className="min-h-screen pb-16 bg-[#F4F5FA]">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <p className="text-[11px] text-slate-400 leading-tight">Function Role Categories</p>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            {isViewMode ? (
              <>
                ...
                <button
                  type="button"
                  onClick={() => navigate(`/function-role-categories/edit/${id}`, { state: { item: data } })}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-6">
                ...
                <div className="mt-2 flex items-center gap-2 flex-wrap">
                  {rolesCount > 0 && (
                    <span className="text-xs text-white/70">
                      {rolesCount} function role{rolesCount === 1 ? '' : 's'}
                    </span>
                  )}
                  {isViewMode && data?.id && (
                    <>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4">
          ...
          <div className="flex items-center gap-2.5 rounded-xl bg-white/80 backdrop-blur-sm px-3.5 py-2.5 border border-slate-200 shadow-sm">
            ...
            <div className="min-w-0">
              <p className="text-[10px] text-slate-500 leading-tight">Function Roles</p>
              <p className="text-sm font-semibold text-slate-700 truncate">
                {rolesCount}
              </p>
            </div>
            ...
            {TABS.map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  {active && (
                    <motion.span
                      layoutId="function-role-category-tab-underline"
      ...
      <ConfirmDialog
        ...
        title="Delete Category"
        message={`Delete category "${categoryName}"? This action cannot be undone.${rolesCount > 0 ? ` It has ${rolesCount} function role${rolesCount === 1 ? '' : 's'} linked.` : ''}`}
      />
    </div>
  );
};
