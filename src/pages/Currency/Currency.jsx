...
const Currency = () => {
  ...
  const normalizeCurrency = (item) => {
    ...
    return {
      id: item.id || item._id,
      currency_name: item.currency_name || item.name || "",
      currency_code: item.currency_code || item.code || "",
      currency_symbol: item.currency_symbol || item.symbol || "",
  ...
  const load = async () => {
    ...
    try {
      ...
      const r = await currencyService.getAll({ limit: 1000 });
      const rawData = r.data?.data || r.data?.results || r.data || [];
      const currencies = Array.isArray(rawData) ? rawData.map(normalizeCurrency) : [];

      const sortedCurrencies = currencies.sort((a, b) => {
        return new Date(b.created_at) - new Date(a.created_at);
      });

      setData(sortedCurrencies);
    } catch (error) {
      console.error("Load error:", error);
      showError(error.message || "Failed to load currencies");
    } finally {
      setLoading(false);
    }
  };
  ...
  // Navigation handlers
  const openAdd = () => {
    navigate('/currencies/add');
  };

  const openEdit = (item) => {
    navigate(`/currencies/edit/${item.id}`);
  };

  const openView = (item) => {
    navigate(`/currencies/view/${item.id}`);
  };
  ...
  const handleStatusToggle = async (row) => {
    ...
    try {
      ...
      await currencyService.update(row.id, updateData);
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "currency_symbol",
      render: (v) => (
        <span className="text-lg font-bold text-gray-800">{v}</span>
      ),
    },
    {
      header: "Status",
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Manage currencies for job listings
          </p>
        </div>
      ...
      <ConfirmDialog
        ...
        message="Delete this currency? This action cannot be undone."
      />
    </div>
  );
};
