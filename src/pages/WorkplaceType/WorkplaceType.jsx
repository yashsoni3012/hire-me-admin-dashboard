// pages/workplace-types/WorkplaceType.jsx
...
const WorkplaceType = () => {
  ...
  // Normalize workplace type data
  const normalizeWorkplaceType = (item) => {
    return {
      id: item.id || item._id,
      name: item.name || "",
  ...
  const load = async () => {
    ...
    try {
      ...
      const r = await workplaceTypeService.getAll({ limit: 1000 });
      const rawData = r.data?.data || r.data?.results || r.data || [];
      const workplaceTypes = Array.isArray(rawData) ? rawData.map(normalizeWorkplaceType) : [];

      const sortedWorkplaceTypes = workplaceTypes.sort((a, b) => {
        return new Date(b.created_at) - new Date(a.created_at);
      });

      setData(sortedWorkplaceTypes);
    } catch (error) {
      console.error("Load error:", error);
      showError(error.message || "Failed to load workplace types");
    } finally {
      setLoading(false);
    }
  };
  ...
  // Navigation handlers
  const openAdd = () => {
    navigate('/workplace-types/add');
  };

  const openEdit = (item) => {
    navigate(`/workplace-types/edit/${item.id}`);
  };

  const openView = (item) => {
    navigate(`/workplace-types/view/${item.id}`);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await workplaceTypeService.delete(deleteId);
  ...
  const handleStatusToggle = async (row) => {
    ...
    try {
      ...
      await workplaceTypeService.update(row.id, updateData);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Workplace Types</h1>
          <p className="text-sm text-gray-500 mt-1">
            Manage workplace types for job listings
          </p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <Table
          ...
          data={paginatedData}
          loading={loading}
          emptyMessage="No workplace types found"
        />
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-5 py-3 border-t border-gray-100">
          <p className="text-xs text-gray-400">
            ...
            {filteredData.length} workplace types
          </p>
