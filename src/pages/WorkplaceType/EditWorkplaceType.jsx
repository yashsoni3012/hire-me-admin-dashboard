...
// import { workplaceTypeService } from '../../services/workplaceType.service';
// import { showSuccess, showError } from '../../utils/toast';

// const EditWorkplaceType = () => {
...
//   // Fetch workplace type data
//   useEffect(() => {
//     const fetchWorkplaceType = async () => {
//       setFetchLoading(true);
//       try {
//         const response = await workplaceTypeService.getById(id);
...
//           setEditItem(data);
//         } else {
//           showError("Workplace type not found");
//           navigate('/workplace-types');
//         }
//       } catch (error) {
//         console.error('Fetch error:', error);
//         showError(error.message || "Failed to load workplace type data");
//         navigate('/workplace-types');
//       } finally {
...
//     {
//       name: "name",
//       label: "Workplace Type Name",
//       type: "text",
//       required: true,
//       placeholder: "e.g. Remote, On-site, Hybrid",
//       help: "Enter the workplace type name",
//     },
//     {
//       name: "status",
//       label: "Status",
...
//       help: "Trending workplace types will be highlighted in the listing",
//     },
//   ];
...
//     name: {
//       required: true,
//       requiredMessage: "Workplace type name is required",
//       minLength: 2,
//       minLengthMessage: "Workplace type name must be at least 2 characters",
//       maxLength: 100,
//       maxLengthMessage: "Workplace type name must be at most 100 characters",
//     },
//   };
...
//       await workplaceTypeService.update(id, submitData);
//       showSuccess("Workplace type updated successfully");
      
//       // Redirect to workplace types list page
//       navigate('/workplace-types');
//     } catch (error) {
//       console.error('Submit error:', error);
//       showError(error.message || error?.response?.data?.message || "Failed to update workplace type");
//     } finally {
//       setLoading(false);
//     }
//   };
...
//     try {
//       await workplaceTypeService.delete(id);
//       showSuccess("Workplace type deleted successfully");
...
//         showError("Cannot delete this workplace type because it is being used in other records.");
//       } else {
//         showError(message || "Failed to delete workplace type");
//       }
//     } finally {
...
//           <p className="text-sm text-gray-400">Loading workplace type data...</p>
//         </div>
//       </div>
//     );
//   }
...
//       submitLabel="Update"
//       navigateTo="/workplace-types"
//       breadcrumb={`Editing: ${editItem?.name || 'Workplace Type'}`}
//       showEdit={false}
//     />
//   );
// };
...
// pages/workplace-types/EditWorkplaceType.jsx
import React, { useState, useEffect } from "react";
import { useNavigate, useParams } from "react-router-dom";
...
const EditWorkplaceType = () => {
  ...
  const validate = () => {
    ...
    if (!formValues.name?.trim()) {
      newErrors.name = "Workplace type name is required";
    } else if (formValues.name.trim().length < 2) {
      newErrors.name = "Workplace type name must be at least 2 characters";
    } else if (formValues.name.trim().length > 100) {
      newErrors.name = "Workplace type name must be at most 100 characters";
    }
    ...
    try {
      ...
      showError(
        error.message ||
          ...
          "Failed to update workplace type"
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (/foreign\s*key|constraint|used|referenced/i.test(message)) {
        showError(
          "Cannot delete this workplace type because it is being used in other records."
        );
      } else {
        showError(message || "Failed to delete workplace type");
      }
  ...
  // ─── Hero helpers ────────────────────────────────────────────
  const heroName = formValues.name?.trim() || "Workplace Type";
        ...
        return (
          <div className="space-y-5">
            <div className="grid grid-cols-1 gap-5">
              <div>
                <FieldLabel required>Workplace Type Name</FieldLabel>
                ...
                {errors.name ? (
                  ...
                  <p className="text-xs text-slate-500 mt-1.5">
                    Enter the workplace type name, e.g. "Remote".
                  </p>
                )}
              </div>
            </div>
          ...
          <div className="space-y-6 max-w-xl">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-2">
                Active workplace types are visible in the listing.
              </p>
            </div>
            ...
            <div className="pt-2 border-t border-slate-100">
              ...
              <p className="text-xs text-slate-400 mt-1.5">
                Trending workplace types are highlighted in the listing.
              </p>
            </div>
          </div>
        );
  ...
  return (
    <div className="min-h-screen pb-16 bg-[#F4F5FA]">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            >
              ...
              {loading ? "Updating..." : "Update Workplace Type"}
            </button>
          </div>
        </div>
      </div>
            ...
            {TABS.map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  {active && (
                    <motion.span
                      layoutId="edit-workplace-type-tab-underline"
      ...
      <ConfirmDialog
        ...
        title="Delete Workplace Type"
        message="Delete this workplace type? This action cannot be undone."
      />
    </div>
  );
};
