...
const JobTypes = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      console.log('Job types response:', r);
      
      const rawData = r?.data?.data || r?.data || r?.results || r || [];
      const jobTypes = Array.isArray(rawData) ? rawData.map(normalizeJobType) : [];
      setData(jobTypes);
    } catch (err) {
      console.error("Load error:", err);
      showError(err.message || "Failed to load job types");
    } finally {
      setLoading(false);
    }
  };
  ...
  // Navigation handlers
  const openAdd = () => {
    navigate('/job-types/add');
  };

  const openEdit = (item) => {
    navigate(`/job-types/edit/${item.id}`);
  };

  const openView = (item) => {
    navigate(`/job-types/view/${item.id}`);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      showError(err.message || "Failed to delete job type");
    } finally {
      setDeleteId(null);
  ...
  // Toggle status - Fixed to send all required fields
  ...
  // Toggle trending - Fixed to send all required fields
