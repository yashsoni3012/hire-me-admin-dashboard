...
//       type: "text",
//       required: true,
//       placeholder: "e.g. Applied, Shortlisted, Interview, Hired",
//       help: "Enter the application status name",
//     },
//     {
//       name: "status",
//       label: "Status Type",
...
//       help: "Trending application statuses will be highlighted in the listing",
//     },
//   ];
...
//       minLength: 2,
//       minLengthMessage: "Status name must be at least 2 characters",
//       maxLength: 100,
//       maxLengthMessage: "Status name must be at most 100 characters",
//     },
//   };
...
//       await applicationStatusService.create(submitData);
//       showSuccess("Application status created successfully");
      
//       // Redirect to application statuses list page
//       navigate('/application-statuses');
//     } catch (error) {
//       console.error('Submit error:', error);
//       showError(error.message || error?.response?.data?.message || "Failed to create application status");
//     } finally {
//       setLoading(false);
//     }
//   };
...
//       breadcrumb="Create a new application status"
//     />
//   );
// };
...
// pages/application-statuses/AddApplicationStatus.jsx
import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
...
const AddApplicationStatus = () => {
  ...
  const validateField = (name, value) => {
    if (name === 'name') {
      if (!value || !value.trim()) return 'Status name is required';
      if (value.trim().length < 2) return 'Status name must be at least 2 characters';
      if (value.trim().length > 100) return 'Status name must be at most 100 characters';
    }
    return null;
  };
  ...
  // ─── Compute hero data ────────────────────────────────────
  const statusName = formValues.name?.trim() || 'New Application Status';
            ...
            <div className="flex flex-col sm:flex-row sm:items-end gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-2 flex items-center gap-2 flex-wrap">
                  <span className="text-xs text-white/50">New Application Status</span>
                </div>
              </div>
            </div>
          </div>
            ...
            {TABS.map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  {active && (
                    <motion.span
                      layoutId="add-application-status-tab-underline"
                    ...
                    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                      ...
                      <div className="p-6">
                        <p className="text-sm text-slate-500">
                          This application status hasn't been created yet. Once created, activity details will appear here.
                        </p>
                      </div>
                    </div>
                  </div>
