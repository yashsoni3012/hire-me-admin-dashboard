...
//         const response = await applicationStatusService.getById(id);
//         const data = response?.data || response;
        
//         if (data) {
//           const formData = {
//             name: data.name || "",
//             status: data.status || (data.is_status ? "active" : "inactive"),
...
//           setViewData(data);
//         } else {
//           showError("Application status not found");
//           navigate('/application-statuses');
//         }
//       } catch (error) {
//         console.error('Fetch error:', error);
//         showError(error.message || "Failed to load application status data");
//         navigate('/application-statuses');
//       } finally {
//         setLoading(false);
//       }
...
//   const handleEdit = () => {
//     navigate(`/application-statuses/edit/${id}`);
//   };
...
//               value === "active" ? "bg-green-500" : "bg-yellow-500"
//             }`}
//           />
...
//           <p className="text-sm text-gray-400">Loading application status details...</p>
//         </div>
//       </div>
//     );
//   }
...
//       onSubmit={() => {}}
//       onEdit={handleEdit}
//       navigateTo="/application-statuses"
//       breadcrumb={`Viewing: ${viewData?.name || 'Application Status'}`}
//       enableEditMode={true}
//       showEdit={true}
//       editLabel="Edit Application Status"
//       cancelLabel="Back to Application Statuses"
//     />
//   );
// };
...
// pages/application-statuses/ViewApplicationStatus.jsx
import React, { useState, useEffect } from 'react';
import { useNavigate, useParams } from 'react-router-dom';
...
const ViewApplicationStatus = () => {
  ...
  useEffect(() => {
    const fetchApplicationStatus = async () => {
      ...
      try {
        ...
        if (data && data.id) {
          // Handle both string status ("active"/"inactive") and boolean is_status
          let statusValue = 'inactive';
          if (data.status === 'active' || data.status === 'inactive') {
            statusValue = data.status;
          } else if (data.is_status !== undefined) {
            statusValue = data.is_status ? 'active' : 'inactive';
          }
  ...
  const handleBack = () => navigate('/application-statuses');
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-[#F4F5FA]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-slate-400">Loading application status details...</p>
        </div>
      </div>
    );
  }
  ...
  // ─── Compute hero data ────────────────────────────────────
  const statusName = statusData.name || 'Application Status';
  const status = statusData.status || 'inactive';
  const isTrending = statusData.is_trending || false;
  const createdDate = statusData.created_at ? formatDate(statusData.created_at) : '—';
  const updatedDate = statusData.updated_at ? formatDate(statusData.updated_at) : '—';

  const initials = statusName
    .split(' ')
    .filter(Boolean)
    .slice(0, 2)
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div>
                ...
                <ReadOnlyValue>
                  <span className="font-medium text-lg">{statusName}</span>
                </ReadOnlyValue>
              </div>
              <div>
                ...
                <div>
                  ...
                  <ReadOnlyValue>
                    {statusData.created_by
                      ? getUserNameCached(statusData.created_by)
                      : '—'}
                  </ReadOnlyValue>
                </div>
                <div>
                ...
                <div>
                  ...
                  <ReadOnlyValue>
                    {statusData.updated_by
                      ? getUserNameCached(statusData.updated_by)
                      : '—'}
                  </ReadOnlyValue>
                </div>
                <div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <p className="text-[11px] text-slate-400 leading-tight">Application Statuses</p>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-2 flex items-center gap-2 flex-wrap">
                  <span className="text-xs text-white/70">ID: #{statusData.id}</span>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-4">
          <div className="flex items-center gap-2.5 rounded-xl bg-white/80 backdrop-blur-sm px-3.5 py-2.5 border border-slate-200 shadow-sm">
            ...
            <div className="min-w-0">
              <p className="text-[10px] text-slate-500 leading-tight">Status Type</p>
            ...
            {TABS.map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  {active && (
                    <motion.span
                      layoutId="view-application-status-tab-underline"
