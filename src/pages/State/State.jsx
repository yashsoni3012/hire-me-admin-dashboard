...
const State = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      const states = Array.isArray(rawData) ? rawData.map(normalizeState) : [];
  ...
  const openEdit = (item) => {
    const stateId = item?.id ?? item?._id;
    if (!stateId) return;
    navigate(`/states/edit/${stateId}`);
  };

  const openView = (item) => {
    const stateId = item?.id ?? item?._id;
    if (!stateId) return;
    navigate(`/states/view/${stateId}`);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await stateService.delete(deleteId);
  ...
  const handleStatusToggle = async (row) => {
    ...
    try {
      ...
      await stateService.update(row.id, updateData);
      showSuccess(
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Manage states for job listings
          </p>
        </div>
      ...
      <ConfirmDialog
        ...
        title="Delete State"
        message="Delete this state? This action cannot be undone."
      />
    </div>
  );
};
