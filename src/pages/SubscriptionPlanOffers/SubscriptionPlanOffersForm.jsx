...
const SubscriptionPlanOffersForm = () => {
  ...
  useEffect(() => {
    ...
    const loadUsers = async () => {
      try {
        ...
        Object.keys(users).forEach((key) => {
          userMap[key] =
            users[key]?.name || users[key]?.username || `User ${key}`;
        });
    ...
    const fetchOffer = async () => {
      ...
      try {
        ...
        const normalizedData = {
          ...
          offer_value: Number.isFinite(Number(item.offer_value))
            ? Number(item.offer_value)
            : 0,
  ...
  const getUserNameCached = (userIdValue) => {
    ...
    return (
      userNameCache[key] || userNameCache[userIdValue] || `User ${userIdValue}`
    );
  };
  ...
  const validate = () => {
    ...
    if (
      formValues.offer_type === "free_trial" &&
      !Number.isInteger(offerValue)
    ) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const subscriptionPlanId =
        mode === "edit" ? Number(selectedPlanIds[0]) : selectedPlanIds;
              ...
              <div className={isAddMode ? "sm:col-span-2" : ""}>
                ...
                {!isViewMode && (
                  <p className="mt-1 text-xs text-slate-400">
                    {isAddMode
                      ? "Select one plan for this offer"
