    ...
    const columns = [
        ...
        },
        {
            ...
            render: (v, row) => {
                ...
                return (
                    <span className="font-semibold text-[#2c0eee]">
                        {row.offer_type === 'percentage' ? `${formatted}%` : `₹${formatted}`}
                    </span>
                );
            },
        },
        ...
        },
        {
            ...
            key: "coupon_required",
            render: (v) => (
                <span className={`text-sm font-medium ${v ? 'text-[#2c0eee]' : 'text-gray-400'}`}>
                    {v ? 'Yes' : 'No'}
                </span>
            ),
        },
        {
            ...
            render: (_, row) => {
                ...
                const isCurrentlyActive = isOfferActive(row);
                return (
                    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${isCurrentlyActive && isActive ? "bg-green-50 text-green-700" : "bg-gray-100 text-gray-500"}`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${isCurrentlyActive && isActive ? "bg-green-500" : "bg-gray-400"}`} />
        ...
        },
      {
       ...
       },
        {
            ...
            render: (id, row) => (
                <div className="flex gap-1">
                    <button
                        onClick={() => navigate(`/subscription-plan-offers/view/${row.id}`, { state: { item: row } })}
                        className="p-1.5 hover:bg-blue-50 text-[#2c0eee] rounded-lg transition-colors"
                        title="View"
                    >
                        <MdVisibility size={16} />
                    ...
                    <button
                        onClick={() => navigate(`/subscription-plan-offers/edit/${row.id}`, { state: { item: row } })}
                        className="p-1.5 hover:bg-gray-100 text-gray-500 rounded-lg transition-colors"
                        title="Edit"
                    >
                        <MdEdit size={16} />
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                ...
                <Button icon={MdAdd} onClick={() => navigate('/subscription-plan-offers/add')}>
                    Add Offer
                </Button>
            </div>
