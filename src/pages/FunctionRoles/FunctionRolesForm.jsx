// // pages/functions/FunctionRolesForm.jsx
...
//                     if (!item) {
//                         const response = await functionRolesService.getById(id);
//                         item = response.data;
//                     }
...
//                     showError("Failed to load function role data");
//                     navigate('/function-roles');
//                 } finally {
...
//         try {
//             // Validate roles_category_id
//             if (!formData.roles_category_id || formData.roles_category_id === '') {
...
//             const allData = await functionRolesService.getAll({ limit: 1000 });
...
//             // if (duplicate) {
//             //     showError('This function role name already exists');
//             //     setLoading(false);
//             //     return;
//             // }
...
//         try {
//             await functionRolesService.delete(id);
...
//             const initialData = {
//                 name: data.name || "",
//                 roles_category_id: data.roles_category_id || "",
...
//     const getTitle = () => {
//         if (mode === 'view') return 'Function Role Details';
//         if (mode === 'edit') return 'Edit Function Role';
//         return 'Add New Function Role';
//     };

//     // Get submit label
//     const getSubmitLabel = () => {
//         if (mode === 'edit') return 'Update Function Role';
//         return 'Create Function Role';
//     };
...
//                     <p className="mt-3 text-gray-500">Loading function role data...</p>
//                 </div>
//             </div>
//         );
//     }
...
//             <div className="min-h-screen bg-gray-50 flex items-center justify-center">
//                 <div className="text-center">
//                     <p className="text-gray-500">Function role not found</p>
//                     <button
//                         onClick={() => navigate('/function-roles')}
...
//             navigateTo="/function-roles"
//             submitLabel={getSubmitLabel()}
//             editLabel="Edit Function Role"
//             deleteLabel="Delete Function Role"
//             loading={loading}
//             showDelete={mode !== 'add'}
//             showEdit={mode === 'view'}
//             enableEditMode={mode === 'view'}
//             breadcrumb={mode === 'view' ? 'Viewing function role details' : mode === 'edit' ? 'Updating function role' : 'Creating new function role'}
//             onEdit={() => navigate(`/function-roles/edit/${id}`, { state: { item: data } })}
//         />
//     );
// };

// export default FunctionRolesForm;
...
const FunctionRolesForm = () => {
    ...
    const fetchData = async () => {
      if ((mode === "edit" || mode === "view") && id) {
        ...
        try {
          ...
          if (!item) throw new Error("Function role not found");
  ...
  const validate = () => {
    ...
    if (!formValues.name?.trim()) {
      ...
      newErrors.name = "Function role name must be at least 2 characters";
    } else if (formValues.name.trim().length > 50) {
      newErrors.name = "Function role name must be at most 50 characters";
    }

    if (!formValues.roles_category_id) {
      newErrors.roles_category_id = "Please select a parent category";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const submitData = {
        ...
        roles_category_id: parseInt(formValues.roles_category_id),
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-[#F4F5FA]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-slate-400">Loading function role data...</p>
        </div>
      </div>
    );
  }
  ...
  if ((mode === "view" || mode === "edit") && !data && !fetchLoading) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-[#F4F5FA]">
        <div className="text-center">
          <p className="text-slate-500">Function role not found</p>
          <button
            onClick={() => navigate("/function-roles")}
  ...
  // ─── Hero helpers ────────────────────────────────────────────
  const heroName = formValues.name?.trim() || "New Function Role";
          ...
          <div className="space-y-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div className="sm:col-span-2">
                <FieldLabel required>Function Role Name</FieldLabel>
                ...
                {errors.name ? (
                  ...
                  <p className="text-xs text-slate-500 mt-1.5">
                    Enter a unique name for the function role.
                  </p>
                )}
              </div>
              ...
              <div className="sm:col-span-2">
                ...
                <div className="relative">
                  ...
                  <select
                    name="roles_category_id"
                    value={formValues.roles_category_id}
                    onChange={handleInputChange}
                    disabled={isViewMode}
                ...
                {errors.roles_category_id ? (
                  ...
                  <p className="text-xs text-slate-500 mt-1.5">
                    Select the parent function role category.
                  </p>
                )}
              </div>
            </div>
          ...
          <div className="space-y-6 max-w-xl">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-2">
                Active function roles are visible in the listing.
              </p>
            </div>
            ...
            <div className="pt-2 border-t border-slate-100">
              ...
              <p className="text-xs text-slate-400 mt-1.5">
                Trending function roles are highlighted in the listing.
              </p>
            </div>
          </div>
        );
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            {mode === "view" ? (
              <button
                type="button"
                onClick={() =>
                  navigate(`/function-roles/edit/${id}`, {
                    state: { item: data },
                  })
                }
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4">
          ...
          <div className="flex items-center gap-2.5 rounded-xl bg-white/80 backdrop-blur-sm px-3.5 py-2.5 border border-slate-200 shadow-sm">
            ...
            <div className="min-w-0">
              ...
              <p className="text-sm font-semibold text-slate-700 truncate">
                {formValues.parent_category_name ||
                  (formValues.roles_category_id
                    ? categoryOptions.find(
                        (opt) =>
                          opt.value === String(formValues.roles_category_id)
                      )?.label || "—"
                    : "—")}
              </p>
            </div>
            ...
            {TABS.map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  {active && (
                    <motion.span
                      layoutId="function-roles-tab-underline"
              ...
              <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto sm:ml-auto">
                ...
                {mode === "view" ? (
                  ...
                  >
                    ...
                    {loading ? "Saving..." : "Update Function Role"}
                  </button>
                )}
              </div>
            </div>
